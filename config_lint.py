...
class ConfigLinter:
    ...
    def __init__(self, strict: bool = False, schema_path: Optional[str] = None):
        ...
        if schema_path and SCHEMA_VALIDATOR_AVAILABLE:
            try:
                self.schema_validator = SchemaValidator(schema_path=schema_path)
            except (FileNotFoundError, ValueError) as e:
                self.results.append(LintResult('error', None, f"Schema error: {e}"))
    ...
    def lint_file(self, filepath: str) -> List[LintResult]:
        ...
        if self.schema_validator and suffix in ('.json', '.yaml', '.yml', '.toml'):
            self._validate_against_schema(content, suffix)
    ...
    def _validate_against_schema(self, content: str, suffix: str) -> None:
        """Validate parsed content against JSON schema."""
        ...
        try:
            ...
            for result in schema_results:
                path_str = ".".join(str(p) for p in result.path) if result.path else "root"
                self.results.append(LintResult(
                    'error',
                    None,
                    f"Schema validation failed at '{path_str}': {result.message}"
                ))
